...
def construct_input(dataset, unlearned_vib, model, loss_fn, args, epoch, noise_flag):
    ...
    patch_slice = (slice(-7, -2), slice(-7, -2))
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        init_random_x[:, :, patch_slice[0], patch_slice[1]] = 0.4
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        if noise_flag == 1:
            updated_x = add_laplace_noise(x, args.eps, 0.2/3, args)  # - input_gradient # if we optimizer step, we don't need subtract
        else:
