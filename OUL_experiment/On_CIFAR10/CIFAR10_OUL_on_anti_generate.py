...
def construct_input(dataset, update_deltas_direction, model, loss_fn, args, epoch):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        # temp = 1 - input_gradient[:, :, -7:-2, -7:-2]
        # input_gradient[:, :, -7:-2, -7:-2] = - temp * epsilon/10
        input_gradient[:, :, -7:-2, -7:-2] = -torch.abs(input_gradient[:, :, -7:-2, -7:-2]) / (torch.norm(input_gradient[:, :, -7:-2, -7:-2], p=1)) * epsilon
        #input_gradient[:, :,  -7:-2, -7:-2] = input_gradient[:, :, -7:-2, -7:-2] / (torch.norm(input_gradient[:, :, -7:-2, -7:-2], p=1)) *60

        l1_norm = torch.norm(input_gradient, p=1)

        updated_x = x - input_gradient
        updated_x = torch.clamp(updated_x, 0, 1)
...
for epoch in range(args.num_epochs):
    dataloader_constructing1, fixed_vib = construct_input(dataloader_constructing1, update_deltas_direction,
                                                            fixed_vib,
                                                            loss_fn, args,
                                                            epoch)
    # vib.train()
