...
def construct_input(dataset, unlearned_vib, model, loss_fn, args, epoch, noise_flag):
    ...
    patch_slice = (slice(-5, -2), slice(-5, -2))
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        init_random_x[:, :, patch_slice[0], patch_slice[1]] = 0.6
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        if noise_flag == 1:
            updated_x = add_laplace_noise(x, args.eps, 1, args)  # - input_gradient # if we optimizer step, we don't need subtract
        else:
...
args.dimZ = 128  # 40 # 2
args.batch_size = 16
args.erased_local_r = 500 #0.000009 #0.006  # when ratio value is 0.000009, the sample is 1. 0.002 # the erased data ratio
...
fixed_vib_2 = copy.deepcopy(shadow_vib)
