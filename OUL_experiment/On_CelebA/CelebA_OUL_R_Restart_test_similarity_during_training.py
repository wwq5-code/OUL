...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        if train_type =="unl":
            backdoor_acc = eva_vib(model, dataloader_erasing_with_tri, args, name='on erased', epoch=999)
            train_acc.append(backdoor_acc)
...
@torch.no_grad()
def eva_vib_with_output(vib, dataloader_erase, args, name='test', epoch=999):
    # first, generate x_hat from trained vae
    vib.eval()
    ...
    for batch_idx, (x, y) in enumerate(dataloader_erase):
        ...
        num_total += len(x)
        change_for_cos = 0
        predicted_labels.extend(logits_y.argmax(dim=1).cpu() - change_for_cos)
        true_labels.extend(y.cpu()-change_for_cos)

    acc = num_correct / num_total
    acc = round(acc, 5)
    print(f'epoch {epoch}, {name} accuracy:  {acc:.4f}, total_num:{num_total}')
    # After the loop, output the predictions and true labels
    predicted_labels = torch.tensor(predicted_labels)
    true_labels = torch.tensor(true_labels)

    # Calculate cosine similarity
    cos_sim = F.cosine_similarity(predicted_labels.float(), true_labels.float(), dim=0)
    print("Cosine Similarity to true y :", cos_sim.item())

    return cos_sim.item(), predicted_labels
...
args.dimZ = 128  # 40 # 2
args.batch_size = 160
args.erased_local_r = 0.005 #0.000009 #0.006  # when ratio value is 0.000009, the sample is 1. 0.002 # the erased data ratio
args.construct_size = 0.005
args.auxiliary_size = 0.005
...
dataloader_erasing_with_tri_no_shuffle = DataLoader(erasing_with_tri, batch_size=args.batch_size, shuffle=False)
...
for epoch in range(args.num_epochs):
    vib.train()
    vib, train_acc = vib_train(dataloader_remaining_and_erasing, vib, loss_fn, reconstruction_function, args, epoch,
...
for epoch in range(10): # args.num_epochs
    vib.train()
    # dataloader_constructing is clean data, dataloader_constructing1 is the synthesized data
    train_type = "unl"
    vib, train_acc = vib_train(dataloader_constructing1, vib, loss_fn, reconstruction_function, args, epoch,
                    train_type)
    print('back acc. during training',train_acc)
    backdoor_acc = eva_vib(vib, dataloader_erasing_with_tri, args, name='on erased', epoch=999)
    ## record the similarity in each round

    cosin_unl, predicted_labels_unl  = eva_vib_with_output(unlearned_vib, dataloader_erasing_with_tri_no_shuffle, args, name='vbu unlearned', epoch=999)
    cosin_oul, predicted_labels_oubl = eva_vib_with_output(vib, dataloader_erasing_with_tri_no_shuffle, args, name='oubl unlearned data', epoch=999)

    cos_sim = F.cosine_similarity(predicted_labels_unl.float(), predicted_labels_oubl.float(), dim=0)
    sim_list.append(cos_sim.item() )
    unlearned_sim_list.append(cosin_unl )

print('forgeability sim list', sim_list)
print('unlearned sim list', unlearned_sim_list)
