...
########
# dataloader_remaining_after_aux
...
for epoch in range(args.num_epochs):
    vib_r.train()
    vib_r = vib_train(dataloader_remaining_after_aux, vib_r, loss_fn, reconstruction_function, args, epoch,
                    train_type)  # dataloader_total, dataloader_w_o_twin
...
end_time = time.time()
running_time = end_time - start_time
print(f'VIB_R Training took {running_time} seconds')



# dataloader_target_with_trigger = DataLoader(target_with_tri, batch_size=args.batch_size, shuffle=True)
vib_r.eval()
acc_r = eva_vib(vib_r, dataloader_remaining_after_aux, args, name='on clean remaining dataset', epoch=999)
backdoor_acc = eva_vib(vib_r, dataloader_erasing_with_tri, args, name='on backdoored erasing dataset', epoch=999)
acc = eva_vib(vib_r, test_loader, args, name='on test dataset', epoch=999)
...
for step, (x, y) in enumerate(dataloader_erasing_with_tri):
    x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
    for (name1, param1), (name2, param2) in zip(vib_r.named_parameters(), vib.named_parameters()):
        # Calculate the difference (delta) needed to update model1 towards model2
