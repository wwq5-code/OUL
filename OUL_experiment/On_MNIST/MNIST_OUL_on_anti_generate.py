...
def construct_input(dataset, unlearned_vib, model, loss_fn, args, epoch):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        x.requires_grad = True
        optimizer_x = torch.optim.Adam([x], 0.1)
        ...
        input_gradient[:, :, width - 3, height - 3] = input_gradient[:, :, width - 3, height - 3] / (torch.norm(input_gradient[:, :, width - 3, height - 3], p=1)) * epsilon/4 * args.laplace_scale #* args.laplace_scale
        input_gradient[:, :, width - 3, height - 4] = input_gradient[:, :, width - 3, height - 4] / (torch.norm(input_gradient[:, :, width - 3, height - 4], p=1)) * epsilon/4 * args.laplace_scale #* args.laplace_scale
        input_gradient[:, :, width - 4, height - 3] = input_gradient[:, :, width - 4, height - 3] / (torch.norm(input_gradient[:, :, width - 4, height - 3], p=1)) * epsilon/4 * args.laplace_scale #* args.laplace_scale
        input_gradient[:, :, width - 4, height - 4] = input_gradient[:, :, width - 4, height - 4] / (torch.norm(input_gradient[:, :, width - 4, height - 4], p=1)) * epsilon/4 * args.laplace_scale #* args.laplace_scale
        l1_norm = torch.norm(input_gradient, p=1)
...
unlearned_vib = prepare_unl(dataloader_erasing_with_tri, unlearned_vib, loss_fn, args, epoch)
unlearned_vib.eval()
