...
def prepare_update_direction_dp(unlearned_vib, model, args):
    ...
    eps_step = args.eps  # per‑step ε  (choose with total ε in mind)
    delta = 1e-6  # target δ
    sigma = C * math.sqrt(2 * math.log(1.25 / delta)) / eps_step  # noise std
    # --------------------------------------------------------------------------
...
def construct_input(dataset, unlearned_vib, model, loss_fn, args, epoch, noise_flag):
    ...
    patch_slice = (slice(-26, -2), slice(-26, -2))
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        if noise_flag == 1:
            updated_x = add_laplace_noise(x, args.eps, 0.2, args)  # - input_gradient # if we optimizer step, we don't need subtract
        else:
            updated_x = x  # - input_gradient # if we optimizer step, we don't need subtract
...
def train_membership_infer(vib_origin, vib_unl, unlearned_dataloader, remaning_dataloader, loss_fn, args):
    ...
    model_output_for_infer = torch.empty(0, 28 * 28).float().to(args.device)
    label_for_infer = torch.empty(0).long().to(args.device)
    for x, y in unl_loader:
        x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)

        _, _, x_hat_o, _, _ = vib_origin(x, mode='with_reconstruction')

        _, _, x_hat_inl, _, _ = vib_unl(x, mode='with_reconstruction')

        x_for_infer = x_hat_o - x_hat_inl
    ...
    infer_model = LinearModel(n_feature=28*28, n_output=2)
...
args.dimZ = 128  # 40 # 2
args.batch_size = 16
args.erased_local_r = 3000 # 0.000017 #0.01  # when ratio value is 0.000017, the sample is 1. 0.002 # the erased data ratio
...
combined_dataset = ConcatDataset([dataset1, erasing_with_tri, dataset2]) # with original dataset2, to show the noise extent.
...
infer_er, infer_loader = train_membership_infer(copy.deepcopy(shadow_vib), copy.deepcopy(unlearned_vib), dataloader_erasing_with_tri, dataloader_erasing, loss_fn, args)
...
infer_er, infer_loader_w = train_membership_infer(copy.deepcopy(fixed_vib_2), copy.deepcopy(vib_for_oubl), dataloader_erasing_with_tri, dataloader_erasing, loss_fn, args)
...
dim_z = 256
temp_grad = torch.empty(0, dim_z).float().to(args.device)
temp_img_cons = torch.empty(0, 1, 28, 28).float().to(args.device)
...
infer_er, infer_loader_wo = train_membership_infer(copy.deepcopy(fixed_vib_2), copy.deepcopy(vib_for_oubl), dataloader_constructing1, dataloader_erasing, loss_fn, args)
...
reconstructor_er_re = evaluate_reconstructor_without_unl_intention(copy.deepcopy(fixed_vib_2), reconstructor_er_re, cons_set_loader, er_reconstruction_set_loader, reconstruction_function, args)
end_time = time.time()
running_time_recon = end_time - start_time
