...
def prepare_unl(erasing_dataset, dataloader_remaining_after_aux, model, loss_fn, args, noise_flag):
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x_e, y_e), (x_r, y_r) in zip(erasing_dataset, dataloader_remaining_after_aux):
            ...
            logits_z_r, logits_y_r, x_hat_r, mu_r, logvar_r = model(x_r, mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
            ...
            KLD_element2 = mu_r.pow(2).add_(logvar_r.exp()).mul_(-1).add_(1).add_(logvar_r).cuda()
            KLD_mean2 = torch.mean(KLD_element2).mul_(-0.5).cuda()

            params_with_hs = calculate_hs_p(KLD_mean2, H_p_q2, optimizer, model)
            ...
            i = 0
            for p_hs, p in zip(params_with_hs, model.parameters()):
                i = i + 1
                # if i==1:
                #     continue
                # print(p_hs.hess)
...
def construct_input(dataset, unlearned_vib, model, loss_fn, args, epoch):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        init_random_x[:, :, width - 3, height - 3] = args.laplace_scale/4
        init_random_x[:, :, width - 3, height - 4] = args.laplace_scale/4
        init_random_x[:, :, width - 4, height - 3] = args.laplace_scale/4
        init_random_x[:, :, width - 4, height - 4] = args.laplace_scale/4
        random_patch = init_random_x[:, :, -4:-2, -4:-2]
        break
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        x2 = x
        x2[:, :, -4:-2, -4:-2] = x2[:, :, -4:-2, -4:-2] + random_patch
        x2 = x2.clamp(0, 1)
        ...
        x[:, :, -4:-2, -4:-2] = x[:, :, -4:-2, -4:-2] + random_patch.detach()
...
start_time = time.time()
unlearned_vib = prepare_unl(dataloader_erasing_with_tri, dataloader_remaining_after_aux, unlearned_vib, loss_fn, args, "no noise")
end_time = time.time()
running_time = end_time - start_time
...
start_time = time.time()
unlearned_vib_with_noise  = prepare_unl(dataloader_erasing_with_tri, dataloader_remaining_after_aux, unlearned_vib, loss_fn, args, "noise")
end_time = time.time()
running_time = end_time - start_time
