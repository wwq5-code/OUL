...
def prepare_unl(erasing_dataset, dataloader_remaining_after_aux, model, loss_fn, args, noise_flag):
    ...
    for epoch in range(args.num_epochs+5):
        ...
        for (x_e, y_e), (x_r, y_r) in zip(erasing_dataset, dataloader_remaining_after_aux):
            ...
            # Calculate the L2-norm
            l2_norm_unl = torch.norm(args.kld_to_org * KLD_mean  -  H_p_q , p=2)
            ...
            loss = (args.kld_to_org * KLD_mean) * unl_rate + self_s_rate *(args.beta * KLD_mean2 + H_p_q2)
...
start_time = time.time()
unlearned_vib = prepare_unl(dataloader_erasing_with_tri, dataloader_remaining_after_aux, unlearned_vib, loss_fn, args, "no noise") #prepare_unl(dataloader_erasing_with_tri, unlearned_vib, loss_fn, args, "no noise")
end_time = time.time()
running_time = end_time - start_time
